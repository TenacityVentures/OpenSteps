    ...
    <section>
      ...
      <div className="space-y-2">
        {feed.map((item) => (
          <div
            key={item.id}
            className="flex items-start gap-3 p-3 rounded-lg bg-white border border-[var(--color-surface3)] text-sm"
          >
            <div className="w-7 h-7 rounded-full bg-[var(--color-surface2)] flex items-center justify-center text-xs font-bold flex-none text-[var(--color-green)]">
              {item.type === 'verify' ? '✓' : item.type === 'edit' ? '✏' : '+'}
            </div>
            <div className="grow min-w-0">
              <div className="text-[var(--color-ink-2)] leading-snug line-clamp-2">
                {item.description}
              </div>
              <div className="text-[10px] font-mono text-[var(--color-ink-3)] mt-1">
