...
export default function GuideTrustPanel({
  ...
  return (
    <div className="space-y-5">
      {/* Trust score */}
      <div className="p-4 rounded-xl bg-white border border-[--color-surface3]">
        <div className="text-xs font-mono uppercase tracking-widest text-[--color-ink-3] mb-2">
          Trust score
        </div>
        <div className="font-serif text-4xl text-[--color-green]">
          {guide.trust_score.toFixed(1)}
        </div>
        <div className="text-xs text-[--color-ink-3] mb-3">/ 10</div>
        <TrustBar score={guide.trust_score} />
        {guide.last_verified_at && (
          <div className="text-xs font-mono text-[--color-ink-3] mt-2">
        ...
        <div className="p-4 rounded-xl bg-white border border-[--color-surface3]">
          ...
          <ul className="space-y-1.5">
            {documents.slice(0, 5).map((d) => (
              <li key={d.id} className="flex items-center gap-2 text-xs text-[--color-ink-2]">
                <span className="w-3.5 h-3.5 rounded border border-[--color-surface3] flex-none" />
                {d.label}
              </li>
            ))}
          </ul>
        ...
        <div className="p-4 rounded-xl bg-white border border-[--color-surface3]">
          ...
          <div className="mt-2 space-y-1">
            {budget.slice(0, 4).map((line) => (
              <div key={line.id} className="flex justify-between text-xs text-[--color-ink-3]">
