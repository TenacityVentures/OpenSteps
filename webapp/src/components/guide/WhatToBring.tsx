...
export default function WhatToBring({ documents }: { documents: DocumentNeeded[] }) {
  ...
  return (
    <section className="p-5 rounded-xl border border-[--color-surface3] bg-[--color-surface2] space-y-3">
      ...
      <ul className="space-y-2">
        {required.map((doc) => (
          <li key={doc.id} className="flex items-center gap-3 text-sm text-[--color-ink-2]">
            <span className="w-4 h-4 rounded border border-[--color-green] flex-none" />
            {doc.label}
          </li>
        ))}
        {optional.map((doc) => (
          <li
            key={doc.id}
            className="flex items-center gap-3 text-sm text-[--color-ink-3]"
          >
            <span className="w-4 h-4 rounded border border-[--color-surface3] flex-none" />
            {doc.label}{' '}
